...
public class ItemActions : Actions
{
    ...
    public override void Act()
    {
        if (giveItem)
        {
            ...
            if (itemOwned > 0)
            {
                if (DataManager.Instance.Inventory.CheckAmount(CurrentItem) > 0)
                {
                    if (CurrentItem.AllowMultple && CurrentItem.Amount <= DataManager.Instance.Inventory.CheckAmount(CurrentItem))
                    {
                        //pass item, invoke yesActions
                    ...
                    {
                        //remove item from inventory, and then invoke yesActions
